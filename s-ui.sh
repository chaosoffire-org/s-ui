...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/chaosoffire-org/s-ui/dev/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/chaosoffire-org/s-ui/dev/install.sh"
...
update_shell() {
    wget -O /usr/bin/s-ui -N --no-check-certificate https://github.com/chaosoffire-org/s-ui/raw/dev/s-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
